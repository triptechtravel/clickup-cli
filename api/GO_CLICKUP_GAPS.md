...
| Archived subtasks are invisible | `GET /task/{id}?include_subtasks=true` returns only **active** subtasks; archived ones are omitted entirely. Verified 2026-07-27 against a live workspace (parent with 3 archived children returned `subtasks: []`). Consequence: `task archive` can detect and report the children it is leaving behind, but `task unarchive --cascade` cannot find them — the flag is a no-op in that direction and the command says so rather than implying it worked. |
