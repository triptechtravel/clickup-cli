...
// CurrencyValue holds a parsed currency custom field value.
type CurrencyValue struct {
	Value      float64
	TypeConfig CurrencyTypeConfig
}

// CurrencyTypeConfig holds the type configuration for currency fields.
...
// EmojiValue holds a parsed emoji custom field value.
type EmojiValue struct {
	Value      int
	TypeConfig EmojiTypeConfig
}

// EmojiTypeConfig holds the type configuration for emoji fields.
type EmojiTypeConfig struct {
	CodePoint string `json:"code_point"`
	Count     int    `json:"count"`
}

// LocationValue holds a parsed location custom field value.
...
// AutomaticProgressValue holds a parsed automatic progress custom field value.
type AutomaticProgressValue struct {
	PercentCompleted float64
	TypeConfig       AutomaticProgressTypeConfig
}

// AutomaticProgressTypeConfig holds the type configuration for automatic progress fields.
...
// ManualProgressValue holds a parsed manual progress custom field value.
...
// ManualProgressTypeConfig holds the type configuration for manual progress fields.
type ManualProgressTypeConfig struct {
	Start int64 `json:"start"`
	End   int64 `json:"end"`
}

// TaskValue holds a parsed task reference custom field value.
...
// TasksValue is a slice of TaskValue.
type TasksValue []TaskValue

// UserValue holds a parsed user reference custom field value.
...
// UsersValue is a slice of UserValue.
type UsersValue []UserValue

// AttachmentValue holds a parsed attachment reference custom field value.
...
// AttachmentsValue is a slice of AttachmentValue.
type AttachmentsValue []AttachmentValue

// LabelsValue holds a parsed labels custom field value.
type LabelsValue struct {
	Values     []LabelOption
	TypeConfig LabelsTypeConfig
}

// LabelsTypeConfig holds the type configuration for labels fields.
type LabelsTypeConfig struct {
	Options []LabelOption
}

// LabelOption represents a single label option.
...
// DropDownValue holds a parsed dropdown custom field value.
type DropDownValue struct {
	Value      DropDownOption
	TypeConfig DropDownTypeConfig
}

// DropDownTypeConfig holds the type configuration for dropdown fields.
...
// GetValue returns the parsed, typed value for a custom field.
