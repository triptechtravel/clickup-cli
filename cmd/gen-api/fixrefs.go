...
func fixSelfRefs(specPath, genFile, fixesFile, pkg string) error {
	...
	fieldSets := make([][]fieldInfo, 0, len(resolved))
	for _, r := range resolved {
		fieldSets = append(fieldSets, r.Fields)
	}

	err = fixesTmpl.Execute(f, map[string]any{
		"Pkg":        pkg,
		"Imports":    importsFor(fieldSets),
		"Resolved":   resolved,
...
// importSpec is the package a field's x-go-type comes from. Zero value means
// the type is built in and needs no import.
type importSpec struct {
	Name string
	Path string
}

// importFor reads the x-go-type-import the spec patch attaches beside every
// x-go-type.
func importFor(prop map[string]any) importSpec {
	raw, ok := prop["x-go-type-import"].(map[string]any)
	if !ok {
		return importSpec{}
	}
...
// importsFor gathers the distinct imports a set of resolved structs needs,
// ordered by path so the generated file does not churn between runs.
func importsFor(fieldSets [][]fieldInfo) []importSpec {
	seen := map[string]importSpec{}
	for _, fields := range fieldSets {
		for _, f := range fields {
			if f.Import.Path != "" {
				seen[f.Import.Path] = f.Import
			}
		}
	}
	if len(seen) == 0 {
...
func schemaToGoType(prop map[string]any) string {
	// x-go-type wins. The spec patch uses it to pin fields ClickUp types
	// inconsistently — every millisecond field is clickup.Millis — and a
	// field that reaches a struct through a $ref must get the same type the
	// OpenAPI codegen gives its inline siblings, or the endpoint it serves
	// goes back to failing on whichever form it was not expecting.
	if goType, ok := prop["x-go-type"].(string); ok && goType != "" {
		return goType
	}
